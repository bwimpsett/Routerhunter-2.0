...
def conectar_ip(ip,rt):
    try:
        ...
        if status != 200:
            raise ValueError(f"The response code was: {status}.  Not what we were looking for.")
        if 'DNS Server Configuration' not in html:
            raise ValueError(f"The response was not returned in the correct format.")
...
def bruteforce_router(ip,user,password,route):
    if port_check(ip,80):
        try:
            ...
            if "200" not in str(get.status_code):
                pass
            print("\n" + "[ + ]" + hour() + bcolors.OKGREEN + "[ ! ] " + url + bcolors.ENDC)
            print("[ + ] " + hour() + bcolors.OKGREEN + "[ ! ] IP: [ " + ip + " ] | DNS1: " + dns1 + " DNS2: " + dns2  + bcolors.ENDC)
